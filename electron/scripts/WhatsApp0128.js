...
async function executeTranslationFlow(inputText) {
    try {
        ...
        let editableDiv = document.querySelector('footer div[aria-owns="emoji-suggestion"][contenteditable="true"]');
        if (editableDiv) {
            editableDiv.textContent = translatedText;
        }
...
async function translateTextAPI(text, fromLang, toLang) {
    ...
    alert("-------------翻译内容："+text+ fromLang + toLang );

    console.log(`调用翻译API: "${text.substring(0, 50)}..." ${fromLang} -> ${toLang}`);

    try {
        console.log('🚀 Calling window.electronAPI.translateText with:', { text, fromLang, toLang });
        const result = await window.electronAPI.translateText({
            text: text,
            local: fromLang,
            target: toLang
        });
        console.log('🔙 window.electronAPI.translateText returned:', result);
        alert("-------------翻译内容后："+result);
        return result;
    } catch (error) {
        console.error('❌ 翻译API调用失败 (Error in WhatsApp.js):', error);
        throw error;
    }
}
